...
def searchfun():
    ...
    def selfun():
        ...
        c=cur.fetchall()
        Label(root3,text="first name: {}".format(y),font='arial 15').grid(row=0,column=0)
        #lb2.insert(END,'first name:')
        #lb2.insert(END,y)
        
        cur.execute('select mname from PHONE_DETAILS where contactID=?',(c[0][0],))
        m=cur.fetchall()
        Label(root3,text='middle name: {}'.format(m[0][0]),font='arial 15').grid(row=1,column=0)
        ...
        web=cur.fetchall()
        Label(root3,text='website URL: {}'.format(web[0][0]),font='arial 15').grid(row=7,column=0)
        #lb2.insert(END,'Website URL:')
        #lb2.insert(END,web[0][0])

        cur.execute('select DOB from PHONE_DETAILS where contactID=?',(c[0][0],))
        d=cur.fetchall()
        Label(root3,text='date of birth: {}'.format(d[0][0]),font='arial 15').grid(row=8,column=0)
        #lb2.insert(END,'Date of Birth:')
        #lb2.insert(END,d[0][0])
        
        #lb2.insert(END,'phone details.......')
        Label(root3,text='PHONE DETAILS.....',font='arial 17').grid(row=9,column=0)

        cur.execute('select contact_type from PHONE_TYPE_DETAILS where contactID=?',(c[0][0],))
        pt=cur.fetchall()
        if pt==0:
            #lb2.insert(END,'Office:')
            Label(root3,text='office: {}',font='arial 15').grid(row==10,column=0)
        elif pt==1:
            #lb2.insert(END,'Home:')
            Label(root3,text='home:{}',font='arial 15').grid(row=10,column=0)
        elif pt==2:
            #lb2.insert(END,'Mobile:')
            Label(root3,text='mobile:{}',font='arial 15').grid(row=10,column=0)
    
        
        cur.execute('select phone_no from PHONE_TYPE_DETAILS where contactID=?',(c[0][0],))
        pn=cur.fetchall()
        Label(root3,text=' {}'.format(pn[0][0]),font='arial 15').grid(row=11,column=0)
        
        #lb2.insert(END,pn[0][0])

        cur.execute('select email_id from MAIL_DETAILS where contactID=?',(c[0][0],))
        mail=cur.fetchall()
        Label(root3,text='EMAIL ADDRESSES.....',font='arial 17').grid(row=12,column=0)
        Label(root3,text=' {}'.format(mail[0][0]),font='arial 15').grid(row=13,column=0)
